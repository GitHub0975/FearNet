...
def make_dataset(dir, class_to_idx,file_list):
    images = []
    #print('here')
    dir = os.path.expanduser(dir)    # 展開含有~的路徑
    set_files = [line.rstrip('\n') for line in open(file_list)]    # 刪除檔名後面的\n並加入陣列中
    for target in sorted(os.listdir(dir)):
        ...
        '''
            root: 當前路徑(有子資料夾的時候會進入子資料夾中)
            fnames: root底下的檔案名稱'''
        for root, _, fnames in sorted(os.walk(d)):    # os.walk()印出資料夾下所有的目錄以及檔案
            for fname in sorted(fnames):
                if is_image_file(fname):
                    dir_file=target+'/'+fname;
                    #print(dir_file)
                    if dir_file in set_files:    # 如果是要的類別才保存下來
